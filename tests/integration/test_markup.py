...
def test_live_regions_use_server_cadence():
    ...
    `throttle` has none (it drops the last change of a burst), and `hx-sync` defaults
    to dropping a refresh that arrives while one is in flight: both lose the tail."""
    ...
    for where, tag in regions:
        ...
        assert 'hx-sync="this:queue last"' in tag, f"{where}: a late refresh would be dropped"
