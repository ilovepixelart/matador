...
Docs prose and a version string are not behavior, so neither arrives through a
failing test the way a feature does. That does not make them uncheckable: a page
nothing links to is a page nobody reads, a link to a renamed page reads as an answer
that exists somewhere, and a release with no upgrading entry is the one a reader
cannot tell apart from a release that broke nothing.
"""
