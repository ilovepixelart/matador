...
The same pin broke toro's 1.0.0 release twice: first as a commit SHA (the action
ships as a Docker image and the runner pulls
`ghcr.io/pypa/gh-action-pypi-publish:<ref>`, which has no manifest for a bare
commit), then as a release tag a year old (its bundled twine rejects the metadata
version `uv build` writes). Both failed in CI after a green build. matador publishes
through the same action, so it gets the same check rather than the same lesson.
"""
...
def test_the_build_refuses_a_tag_that_does_not_match_what_it_built():
    """The tag starts the release and the module supplies the version, and nothing
