"""Unit: the workflows, checked rather than discovered.

The publish pin broke toro's 1.0.0 release twice: first as a commit (the action
generates a Docker action whose image is `ghcr.io/<repo>:<ref>`, and nothing is
published under a bare commit), then as a release tag a year old (its bundled twine
rejected the metadata version `uv build` writes). matador publishes through the same
action, so it gets the same checks rather than the same lessons.
"""
